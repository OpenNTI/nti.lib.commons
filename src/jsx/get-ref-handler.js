...
const setRef = (ref, x) =>
	ref && typeof ref === 'function' ? ref(x) : (ref.current = x);
...
export function getRefHandler(parentRef, localRef) {
	...
	if (!isRef(parentRef)) {
		if (parentRef) {
			...
			console.error(
				`Unsupported value (${typeof parentRef}: ${parentRef}) has been dropped. Use a ref object or callback instead.`
			);
		}
		return localRef;
	}
